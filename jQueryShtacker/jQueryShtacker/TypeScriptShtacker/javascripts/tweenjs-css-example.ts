...
window.onload = () => {
	...
	var tweenCssExample: TweenJSCssExample = new TweenJSCssExample();
...
class TweenJSCssExample 
{
	// Membes
	private m_ColorSeed:number = 0;
	...
	{		
		...
		while (--count >= 0) {
			...
			createjs.Tween.get(box, { loop: true }, true).set({ opacity: "0" }, box.style).wait(Math.random() * 1000 + 1 | 0).call((tween: createjs.Tween) => { this.updateColor(tween); }).to({top:y,left:x,width:16,height:4,opacity:1},Math.random()*1500+1000, this.easeIn );
		}
	...
	//		PRIVATE METHODS
	// --------------------------------------------------------------------------------------------------------------------
	private updateColor(tween:createjs.Tween)
	{		
