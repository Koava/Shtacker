...
class Game {
    ...
    startPlay(): void {
        ...
        //createjs.Tween.get(circle1).to({ x: 19 }, this.speed).to({ x: 342 }, this.speed);
        //createjs.Tween.get(circle2).to({ x: 58 }, this.speed).to({ x: 381 }, this.speed);

        //circle1.x = 342;
        //circle2.x = 381;

        //createjs.Tween.get(circle1, { loop: true }).to({ x: 19 }, this.speed).to({ x: 342 }, this.speed).on;
        //createjs.Tween.get(circle2, { loop: true }).to({ x: 58 }, this.speed).to({ x: 381 }, this.speed);
